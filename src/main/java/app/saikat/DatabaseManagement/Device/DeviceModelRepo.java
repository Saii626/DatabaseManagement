...
public class DeviceModelRepo extends AbstractBaseRepository {
	...
	public DeviceModel getDeviceWithID(UUID id) {
		TypedQuery<DeviceModel> getById = entityManager.createQuery("FROM DeviceModel M WHERE M.id = :id", DeviceModel.class);
		return getById.setParameter("id", id).getSingleResult();
	}

	public DeviceModel getDeviceWithName(String name) {
		TypedQuery<DeviceModel> getById = entityManager.createQuery("FROM DeviceModel M WHERE M.name = :name", DeviceModel.class);
		return getById.setParameter("name", name).getSingleResult();
	}
}